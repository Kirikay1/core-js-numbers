...
function getAverage(value1, value2) {
  const bvalue1 = BigInt(value1);
  const bvalue2 = BigInt(value2);

  return Number((bvalue1 + bvalue2) / 2n);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalСomposition = x1 * x2 + y1 * y2;

  const vectorA = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorB = Math.sqrt(x2 ** 2 + y2 ** 2);

  const cos0 = scalСomposition / (vectorA * vectorB);
...
function getSumOfDigits(num) {
  const asd = num.toString().split('');
  let a = 0;

  for (let i = 0; i < asd.length; i += 1) {
    a += Number(asd[i]);
  }
